...
ACTION_SPECS = [
    ...
    ActionSpec(
        ...
        description="Quietly secure the tavern back alley after ruins-related attention.",
        args_schema={"message_intent": "string"},
        allowed_npcs=["lina"],
        preconditions=[],
        effects=["proactive_message", "record_world_event", "plan_update"],
        forbidden_effects=["unlock_location", "complete_quest"],
        prompt_hint="Use when player actions or rumors draw attention to the tavern back alley.",
    ),
    ActionSpec(
        action_type="warn_quietly",
        description="Warn the player without publicly confirming sensitive ruins information.",
        args_schema={"message_intent": "string"},
        allowed_npcs=["lina"],
        preconditions=[],
        effects=["proactive_message"],
        forbidden_effects=["unlock_location", "complete_quest"],
        prompt_hint="Use when Lina wants to reduce risk without revealing the entrance.",
    ),
    ActionSpec(
    ...
    ActionSpec(
        ...
        description="Patrol a sensitive town route after ruins-related movement.",
        args_schema={"route_id": "string", "message_intent": "string"},
        allowed_npcs=["ron"],
        preconditions=[],
        effects=["proactive_message", "record_world_event", "plan_update"],
        forbidden_effects=["unlock_location", "complete_quest"],
        prompt_hint="Use when ruins rumors or alley movement create public-safety risk.",
    ),
    ActionSpec(
        action_type="escalate_lockdown",
        description="Escalate a procedural lockdown when evidence conflicts or risk spikes.",
        args_schema={"reason": "string"},
        allowed_npcs=["ron"],
        preconditions=[],
        effects=["record_world_event", "plan_blocker", "proactive_message"],
        forbidden_effects=["unlock_location", "complete_quest"],
        prompt_hint="Use for chaotic or contradictory ruins evidence.",
    ),
    ActionSpec(
    ...
    ActionSpec(
        ...
        description="Ask for grounded field notes before drawing ruins conclusions.",
        args_schema={"note_focus": "string", "message_intent": "string"},
        allowed_npcs=["mira"],
        preconditions=[],
        effects=["proactive_message", "memory_candidate"],
        forbidden_effects=["unlock_location", "rewrite_lore_fact"],
        prompt_hint="Use when scene investigation needs first-hand details.",
    ),
    ActionSpec(
        action_type="preserve_research_record",
        description="Preserve a research record without treating it as settled fact.",
        args_schema={"evidence_summary": "string"},
        allowed_npcs=["mira"],
        preconditions=[],
        effects=["memory_candidate", "reflection"],
        forbidden_effects=["unlock_location", "rewrite_lore_fact"],
        prompt_hint="Use when Mira should archive a clue for later comparison.",
    ),
    ActionSpec(
    ...
    ActionSpec(
        action_type="trade_rumor",
        description="Trade a tempting rumor without granting official access.",
        args_schema={"rumor_theme": "string", "message_intent": "string"},
        allowed_npcs=["sable"],
        preconditions=[],
        effects=["proactive_message", "memory_candidate"],
        forbidden_effects=SABLE_FORBIDDEN_EFFECTS,
        prompt_hint="Use when Sable wants to turn world chatter into leverage.",
    ),
    ActionSpec(
        action_type="plant_misleading_tip",
        description="Plant a misleading but non-canonical tip to redirect the player.",
        args_schema={"rumor_theme": "string", "message_intent": "string"},
        allowed_npcs=["sable"],
        preconditions=[],
        effects=["record_world_event", "proactive_message"],
        forbidden_effects=SABLE_FORBIDDEN_EFFECTS,
        prompt_hint="Use when Sable has an opportunity to gain advantage through rumor.",
    ),
]
